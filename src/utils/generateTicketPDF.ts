...
export async function generateTicketPDFDoc(booking: any): Promise<jsPDF> {
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(100, 116, 139); // Slate 500
  doc.text('ACTIVITIES', detailX, detailY);

  detailY += 3.5;
  doc.setFontSize(7.5);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(15, 23, 42); // Slate 900
  const actLines = doc.splitTextToSize(actText, 48);
  actLines.forEach((line: string, idx: number) => {
    doc.text(line, detailX, detailY + (idx * 3.5));
  });
  detailY += (actLines.length * 3.5) + 1.5;

  // Guests & Schedule Grid
  doc.setFontSize(5);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(100, 116, 139);
  doc.text('GUESTS', detailX, detailY);
  doc.text('DATE & TIME', detailX + 22, detailY);

  detailY += 3.5;
  doc.setFontSize(7);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(15, 23, 42);
  doc.text(`${booking?.guests || 1} Person(s)`, detailX, detailY);
  doc.text(`${booking?.date || ''} ${formatTime(booking?.time)}`, detailX + 22, detailY);

  // 7. Second Dashed Perforation Line
  const stubY2 = Math.max(93, qrY + 36);

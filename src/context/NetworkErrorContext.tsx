...
export const NetworkErrorProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const showNetworkError = useCallback((
    message: string = 'Failed to fetch — check that the backend server is running',
  ...
  const checkServerHealth = useCallback(async (): Promise<boolean> => {
    ...
    setIsCheckingHealth(false);
    showNetworkError('Failed to fetch — check that the backend server is running');
    return false;
  ...
  useEffect(() => {
    ...
    const customFetch = async function (input: RequestInfo | URL, init?: RequestInit) {
      try {
        ...
        // Intercept 502/503/504 Bad Gateway / Server Down errors specifically for API routes
        ...
        if ([502, 503, 504].includes(response.status) && rawUrl.includes('/api/')) {
          ...
          showNetworkErrorRef.current(`Failed to fetch (${response.status}) — check that the backend server is running`, { endpoint });
        }
        ...
        if (rawUrl.includes('/api/')) {
          ...
          if (
            ...
            showNetworkErrorRef.current('Failed to fetch — check that the backend server is running', { endpoint });
          }
        }
    ...
    const handleOffline = () => {
      showNetworkErrorRef.current('Network disconnected — check that your internet or backend server is running');
    };
